...
// 解析dom 列表
func (obj *Client) ParseLis(options ...ListOption) [][]*NodeA {
	option := ListOption{}
	if len(options) > 0 {
		option = options[0]
	}
	aas := getA(obj.Find("body").Childrens(), []string{"body"})
	for _, aa := range aas {
...
// 对比html 然后删除 obj 中重复的部分，然后返回 obj 的结果
func (obj *Client) ParseLiss(html *Client, options ...ListOption) [][]*NodeA {
	result := obj.ParseLis(options...)
	result2 := html.ParseLis(options...)
	delIndex := []int{}
...
// 解析dom 内容
func Content(html *Client) *Client {
...
func (obj *NodeA) MainNode() *Client { //返回主节点
...
func getA(htmls []*Client, paths []string) []*NodeA {
	results := []*NodeA{}
